...
function initialiseHandler(id, sensor)
{
	...
	if(id==0)
	{
		switch(sensor){
			...
			case "Temperature":
				sensortag0.temperatureCallback(temperatureHandler, 1000);
			case "Humidity":
				sensortag0.humidityCallback(humidityHandler, 1000);
			case "Barometer":
				sensortag0.barometerCallback(barometerHandler, 1000);
			case "Accelerometer":
				sensortag0.accelerometerCallback(accelerometerHandler, 1000);
			case "Magnetometer":
				sensortag0.magnetometerCallback(magnetometerHandler, 1000);
			case "Gyroscope":
				sensortag0.gyroscopeCallback(gyroscopeHandler, 1000);
			case "Luxometer":
				sensortag0.luxometerCallback(luxometerHandler, 1000);
		}
	} 
	...
	if(id==1)
	{
		switch(sensor){
			...
			case "Temperature":
				sensortag1.temperatureCallback(temperatureHandler1, 1000)
			case "Humidity":
				sensortag1.humidityCallback(humidityHandler1, 1000)
			case "Barometer":
				sensortag1.barometerCallback(barometerHandler1, 1000)
			case "Accelerometer":
				sensortag1.accelerometerCallback(accelerometerHandler1, 1000)
			case "Magnetometer":
				sensortag1.magnetometerCallback(magnetometerHandler1, 1000)
			case "Gyroscope":
				sensortag1.gyroscopeCallback(gyroscopeHandler1, 1000)
			case "Luxometer":
				sensortag1.luxometerCallback(luxometerHandler1, 1000)
		}
	}
}
...
{
	console.log("DEBUG - Looking up " + systemID)
...
{ 
	...
	if(displayValuesFlag)
	{
		...
		if(expConfigData.sensorTags[id].sensors[sensorType].graph.graphdisplay == "on" && graphDisplay[sensorType+id])
		{
			...
			var ymin = datapoints[sensorType].ymin 
			console.log("DEBUG - sending the min values" + xmin + " and " + ymin)
			if(IR&&ambient){
				showGraphicData(sensorType+id+"GraphCanvas", graphType, graphTitle, xAxisTitle, yAxisTitle, xmin, ymin, datapoints[sensorType].x[id], "Ambient Temperature (C)", datapoints[sensorType].y[id], "Target (IR) Temerature (C)");
			}else if (IR){
				showGraphicData(sensorType+id+"GraphCanvas", graphType, graphTitle, xAxisTitle, yAxisTitle, xmin, ymin,  datapoints[sensorType].y[id], "Target (IR) Temerature (C)");
			}else if (ambient){
				showGraphicData(sensorType+id+"GraphCanvas", graphType, graphTitle, xAxisTitle, yAxisTitle, xmin, ymin,  datapoints[sensorType].x[id], "Ambient Temperature (C)");
			}
		}
	}
}
...
function gyroscopeHandler(data)
{
	...
	xyzDisplay(0,"Gyroscope", values,x,y,z);
}
...
function gyroscopeHandler1(data)
{
	...
	xyzDisplay(1,"Gyroscope", values,x,y,z);
}
